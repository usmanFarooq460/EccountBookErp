...
import { DataSourceDTO, PreProductionLabModel, PreProductionLabDetailModel } from "../pre-production-lab.model";
import { PreProductionLabService } from "../pre-production-lab.service";
import notify from "devextreme/ui/notify";
import { CommonMethodsForCombos } from "src/app/shared/Base/common-methods-for-combos";
import { BaseActions } from "src/app/shared/Base/BaseActions";
@Component({
  selector: "app-pre-production-lab-form",
  templateUrl: "./pre-production-lab-form.component.html",
  styleUrls: [],
})
export class PreProductionLabFormComponent extends BaseActions implements OnInit {
  @ViewChild(DxDataGridComponent, { static: false })
  dataGrid: DxDataGridComponent;
  @ViewChild("PreProductionLabForm", { static: false }) //POF = PurchaseOrderForm
  PreProductionLabForm: DxFormComponent;
  PreProductionLabFormData: PreProductionLabModel; //PurchaseOrderData => formObject
  @ViewChild("PreProductionLabDetailForm", { static: false }) //POF = PurchaseOrderDetailForm
  PreProductionLabDetailForm: DxFormComponent;
  PreProductionLabDetailFormData: PreProductionLabDetailModel; //SaleOrderDetail = formObject
  // companyList = [];
  branchList = [];
  projectList = [];
  ...
  async initForm() {
    this.PreProductionLabFormData = new PreProductionLabModel();
    this.PreProductionLabDetailFormData = new PreProductionLabDetailModel();
    this.PreProductionLabFormData.ReportDate = new Date();
    if (this.ParamsId > 0) {
      this.setFields4editMode();
    } else {
      let docNo = await this.generateDocNumber();
      this.PreProductionLabFormData.ReportDocNo = docNo[0].DocNo;
    }
  }
